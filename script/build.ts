...
// import frontMatter from 'front-matter' // TEMP: unused
...
// TEMP: commented interfaces and imports unused after skipping license/metadata
...
// TEMP: Skip emoji packaging (submodule not initialized) - comment out to enable
...
// TEMP: Skip license metadata (submodule not init) - comment out to enable
...
function packageApp() {
  ...
  return packager({
    ...
    // 设置 ELECTRON_ZIP_DIR 环境变量时直接从本地目录读取 Electron zip，
    // 避免构建时重复下载 144MB 的 Electron 压缩包
...
// TEMP: copyEmoji skipped (submodule not init), comment out to fix TS6133
// function copyEmoji() {
...
// TEMP: generateLicenseMetadata skipped (submodule not init) - comment out to fix TS6133
