...
/**
 * On Windows the custom-protocol callback can reach us as a positional
 * command-line argument like `x-github-client://oauth/...?code=...&state=...`.
 * If we let Electron's bootstrap see that argument it will try to `require()`
 * it as an app path, producing "Unable to find Electron app at ...?code=...".
 * We therefore pre-filter the argv array here, after possibleProtocols is
 * defined, moving any protocol URL into a `--protocol-launcher URL` pair so
 * that the subsequent minimist parsing handles it without Electron trying to
 * load it as code.
 *
 * We also strip any protocol URL from argv[1] *specifically* before Windows
 * Squirrel/handleSquirrelEvent are evaluated. That way, when GitHub Desktop
 * registers the protocol handler as `"...\\GitHubDesktop.exe" "%1"` (without
 * the `--protocol-launcher` sentinel), a first-launch / first-instance callback
 * URL in argv[1] is NEVER fed to app.getAppPath() / Electron's module loader.
 * This is the primary fix for the user-facing "Error launching app" dialog
 * that appears when doing OAuth from the welcome wizard on first run.
 */
(function sanitizePositionalProtocolArgs() {
  if (process.argv.length < 2) {
    return
  }
  const knownPrefixes = Array.from(possibleProtocols, p => `${p}://`)
  // Also accept scheme:/?query (single slash) and scheme:?query (no slashes)
  // produced by some GitHub OAuth redirect_uri configurations or Windows URL
  // protocol shells.
  const knownColonPrefixes = Array.from(possibleProtocols, p => `${p}:`)
  const isProtocolUrl = (arg: unknown): arg is string => {
    ...
    for (const p of knownColonPrefixes) {
      if (!arg.startsWith(p)) continue
      const rest = arg.substring(p.length)
      if (!rest) continue
  ...
  if (urls.length) {
    ...
    for (const u of urls) {
      process.argv.push('--protocol-launcher', u)
    }
  }
})()
...
async function handleCommandLineArguments(argv: string[]) {
  ...
  if (__WIN32__ && args['protocol-launcher'] === true) {
    ...
    const strictPrefixes = Array.from(possibleProtocols, p => `${p}://`)
    const loosePrefixes = Array.from(possibleProtocols, p => `${p}:`)
    const matchingUrl = argv.find(arg => {
      const isStrict = strictPrefixes.some(p => arg.startsWith(p))
      const isLoose = loosePrefixes.some(
        p => arg.startsWith(p + '/') || arg.startsWith(p + '?')
      )
      if (isStrict || isLoose) {
        try {
          // Some OAuth redirects arrive as `scheme:/?code=...` (single slash)
          // which Node's WHATWG URL parser may reject. Fall back to url.parse
          // via require() to validate structure.
          if (isStrict) {
            new URL(arg)
          } else {
            // eslint-disable-next-line @typescript-eslint/no-var-requires
            require('url').parse(arg)
          }
          return true
        } catch (e) {
