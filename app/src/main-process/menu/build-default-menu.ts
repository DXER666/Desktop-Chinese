...
import {
  ...
  resolveMenuLanguageCode,
} from '../../lib/i18n/menu-locale-dict'
...
export function buildDefaultMenuTemplate({
  ...
  if (__DARWIN__) {
    template.push({
      ...
      submenu: [
        {
          label: 'About GitHub Desktop Chinese',
  ...
  template.push({
    ...
    submenu: [
      ...
      },
      {
        ...
        accelerator: __DARWIN__ ? 'Control+Command+F' : 'F11',
        click(_item: any, focusedWindow: Electron.BaseWindow | undefined) {
          const window =
  ...
  const resolvedLang = resolveMenuLanguageCode(uiLanguage)
  applyMenuTranslations(template, resolvedLang)
