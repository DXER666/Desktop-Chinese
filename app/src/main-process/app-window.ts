...
export class AppWindow {
  ...
  public load() {
    ...
    this.window.webContents.once('did-finish-load', () => {
      if (shouldAutomaticallyOpenDevTools()) {
...
/**
 * 是否在加载完成后自动打开 DevTools。
 *
 * 只在用户显式启用时打开（设置环境变量 DESKTOP_OPEN_DEVTOOLS=1），
 * 避免 dev 构建每次启动都在右侧弹出控制台影响日常使用。
 * 同时真打包（isPackaged）时即便设置了环境变量也不打开。
 */
function shouldAutomaticallyOpenDevTools(): boolean {
  if (app.isPackaged) {
    return false
  }
  const value = process.env.DESKTOP_OPEN_DEVTOOLS
