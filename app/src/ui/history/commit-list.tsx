...
class CommitListInternal extends React.Component<
  ICommitListProps & WithTranslation,
  ICommitListState
> {
  ...
  public constructor(props: ICommitListProps & WithTranslation) {
    super(props)
  ...
  private renderRowFocusTooltip = (indexPath: RowIndexPath | undefined) => {
    ...
    return (
      <div className="commit-list-item-tooltip list-item-tooltip">
        ...
        <div>
          <div className="label">
            {t('history.commitTooltip.dateLabel', 'Date:')}
          </div>
          {absoluteDate}
        </div>
  ...
  private getContextMenuForSingleCommit(
    ...
    let viewOnGitHubLabel = t('contextMenu.viewOnGitHub', {
    ...
    if (
      ...
      viewOnGitHubLabel = t('contextMenu.viewOnGitHubEnterprise', {
        defaultValue: 'View on GitHub Enterprise',
      })
    }
    ...
    if (canBeAmended) {
      items.push({
        label: __DARWIN__
          ? t('contextMenu.amendCommitDarwin', { defaultValue: 'Amend Commit…' })
          : t('contextMenu.amendCommitOther', { defaultValue: 'Amend commit…' }),
    ...
    if (canBeUndone) {
      items.push({
        label: __DARWIN__
          ? t('contextMenu.undoCommitDarwin', { defaultValue: 'Undo Commit…' })
          : t('contextMenu.undoCommitOther', { defaultValue: 'Undo commit…' }),
        action: () => {
    ...
    items.push({
      label: __DARWIN__
        ? t('contextMenu.resetToCommitDarwin', {
            defaultValue: 'Reset to Commit…',
          })
        : t('contextMenu.resetToCommitOther', {
            defaultValue: 'Reset to commit…',
          }),
      action: () => {
    ...
    items.push({
      label: __DARWIN__
        ? t('contextMenu.checkoutCommitDarwin', {
            defaultValue: 'Checkout Commit',
          })
        : t('contextMenu.checkoutCommitOther', {
            defaultValue: 'Checkout commit',
          }),
      action: () => {
    ...
    items.push({
      label: __DARWIN__
        ? t('contextMenu.reorderCommitDarwin', {
            defaultValue: 'Reorder Commit',
          })
        : t('contextMenu.reorderCommitOther', {
            defaultValue: 'Reorder commit',
          }),
      action: () => {
    ...
    items.push(
      {
        label: __DARWIN__
          ? t('contextMenu.revertChangesInCommitDarwin', {
              defaultValue: 'Revert Changes in Commit',
            })
          : t('contextMenu.revertChangesInCommitOther', {
              defaultValue: 'Revert changes in commit',
            }),
        action: () => {
      ...
      {
        label: __DARWIN__
          ? t('contextMenu.createBranchFromCommitDarwin', {
              defaultValue: 'Create Branch from Commit',
            })
          : t('contextMenu.createBranchFromCommitOther', {
              defaultValue: 'Create branch from commit',
            }),
        action: () => {
      ...
      {
        label: t('contextMenu.createTag', { defaultValue: 'Create Tag…' }),
    ...
    const darwinTagsLabel =
      commit.tags.length > 1
        ? t('contextMenu.copyTagsDarwin', { defaultValue: 'Copy Tags' })
        : t('contextMenu.copyTagDarwin', { defaultValue: 'Copy Tag' })
    const windowTagsLabel =
      commit.tags.length > 1
        ? t('contextMenu.copyTagsOther', { defaultValue: 'Copy tags' })
        : t('contextMenu.copyTagOther', { defaultValue: 'Copy tag' })
    items.push(
      {
        label: __DARWIN__
          ? t('contextMenu.cherryPickCommitDarwin', {
              defaultValue: 'Cherry-pick Commit…',
            })
          : t('contextMenu.cherryPickCommitOther', {
              defaultValue: 'Cherry-pick commit…',
            }),
      ...
      { type: 'separator' },
      {
        label: t('contextMenu.copySHA', { defaultValue: 'Copy SHA' }),
    ...
    return [
      {
        label: __DARWIN__
          ? t('contextMenu.cherryPickCommitDarwin', {
              count,
              defaultValue: `Cherry-pick ${count} Commits…`,
            })
          : t('contextMenu.cherryPickCommitOther', {
              count,
              defaultValue: `Cherry-pick ${count} commits…`,
      ...
      },
      {
        label: __DARWIN__
          ? t('contextMenu.reorderCommitDarwin', {
              defaultValue: `Reorder ${count} Commits…`,
              count,
            })
          : t('contextMenu.reorderCommitOther', {
              defaultValue: `Reorder ${count} commits…`,
...
/**
 * 列表组件对外保持不变的命名：用 withTranslation 包一层以便 tooltip 里的
 * 「Date:」等文案可以根据 UI 语言本地化。
 *
 * 比较页面（compare.tsx）里通过 `React.createRef<CommitListClass>()` 拿到实例
 * 并调用 `focus()`，所以把内部类单独导出来作为 ref 的类型；对 JSX 里的
 * `<CommitList />` 仍使用下面这个 wrapped 过的组件即可。
 */
export type CommitListClass = CommitListInternal

export const CommitList = withTranslation()(
  CommitListInternal
) as unknown as React.ComponentClass<ICommitListProps>
