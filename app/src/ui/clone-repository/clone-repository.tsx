...
class CloneRepositoryInternal extends React.Component<
  ICloneRepositoryProps & WithTranslation,
...
> {
  ...
  public constructor(props: ICloneRepositoryProps & WithTranslation) {
    super(props)
  ...
  public render() {
    ...
    return (
      <Dialog
        ...
        title={__DARWIN__ ? t('cloneRepository.titleDarwin') : t('cloneRepository.titleOther')}
        onSubmit={this.clone}
  ...
  private renderFooter() {
    ...
    return (
      <DialogFooter>
        <OkCancelButtonGroup
          okButtonText={t('cloneRepository.okButtonText')}
  ...
  private renderSignIn(tab: CloneRepositoryTab) {
    const { t } = this.props
    const signInTitle = __DARWIN__ ? t('cloneRepository.signInDotComTitle') : t('cloneRepository.signInEnterpriseTitle')
    switch (tab) {
      case CloneRepositoryTab.DotCom:
        return (
          <CallToAction actionTitle={signInTitle} onAction={this.signInDotCom}>
            <div>
              {t('cloneRepository.signInDotComMsg')}
            </div>
          </CallToAction>
        )
      case CloneRepositoryTab.Enterprise:
        return (
          ...
          >
            <div>
              {t('cloneRepository.signInEnterpriseMsg')}
            </div>
          </CallToAction>
        )
      case CloneRepositoryTab.Generic:
  ...
  private onChooseWithSaveDialog = async (): Promise<string | undefined> => {
    ...
    const path = await showSaveDialog({
      buttonLabel: t('cloneRepository.selectButtonLabel'),
      nameFieldLabel: t('cloneRepository.cloneAsLabel'),
      showsTagField: false,
...
/**
 * 用 withTranslation 包装后导出的克隆仓库对话框组件。
 */
export const CloneRepository = withTranslation()(
  CloneRepositoryInternal
) as React.ComponentClass<ICloneRepositoryProps>
