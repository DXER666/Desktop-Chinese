...
class CloneableRepositoryFilterListInternal extends React.PureComponent<
  ICloneableRepositoryFilterListProps & WithTranslation
> {
  /**
  ...
  public render() {
    const { t, repositories, account, selectedItem } = this.props
    ...
    return (
      <SectionFilterList<ICloneableRepositoryListItem>
        ...
        placeholderText={t('cloneRepository.filterYourRepos')}
  ...
  private getYourRepositoriesLabel = () => {
    const { t } = this.props
    return t('cloneRepository.yourRepositories')
  }
  ...
  private renderNoItems = () => {
    const { t, loading, repositories, account } = this.props
    ...
    return (
      <div className="no-items empty-repository-list">
        <div>
          {t('cloneRepository.noReposPlaceholder')}
        </div>
      </div>
    )
  }
...
/**
 * 用 withTranslation 包装后导出的仓库筛选列表组件。
 */
export const CloneableRepositoryFilterList = withTranslation()(
  CloneableRepositoryFilterListInternal
) as React.ComponentClass<ICloneableRepositoryFilterListProps>
