...
export function getNoResultsMessage(
  ...
  if (filters.isIncludedInCommit) {
    activeFilters.push(t ? t('changes.includedInCommit') : 'Included in commit')
  }

  if (filters.isExcludedFromCommit) {
    activeFilters.push(
      t ? t('changes.excludedFromCommit') : 'Excluded from commit'
    )
  }

  if (filters.isNewFile) {
    activeFilters.push(t ? t('changes.newFiles') : 'New files')
  }

  if (filters.isModifiedFile) {
    activeFilters.push(t ? t('changes.modifiedFiles') : 'Modified files')
  }

  if (filters.isDeletedFile) {
    activeFilters.push(t ? t('changes.deletedFiles') : 'Deleted files')
  }
  ...
  if (t) {
    return t('changes.noResultsMessage', { filters: filterList })
  }
