...
export class FilterChangesListInternal extends React.Component<
  IFilterChangesListProps & WithTranslation,
...
> {
  ...
  private renderChangedFile = (
    ...
    const checkboxTooltip = isUncommittableSubmodule
      ? t('changes.submoduleNotCommittedTooltip')
      : isPartiallyCommittableSubmodule
      ? t('changes.submodulePartialTooltip')
      : undefined
  ...
  private getDiscardChangesMenuItemLabel = (files: ReadonlyArray<string>) => {
    ...
    const label =
      files.length === 1
        ? __DARWIN__
          ? t('changes.discardChangesDarwin')
          : t('changes.discardChangesOther')
        : __DARWIN__
        ? t('changes.discardChangesMultipleDarwin', { count: files.length })
        : t('changes.discardChangesMultipleOther', { count: files.length })
  ...
  private onContextMenu = (event: React.MouseEvent<any>) => {
    ...
    const stashAllChangesLabel = __DARWIN__
      ? t('changes.stashAllChangesDarwin')
      : t('changes.stashAllChangesOther')
    const confirmStashAllChangesLabel = __DARWIN__
      ? t('changes.confirmStashAllChangesDarwin')
      : t('changes.confirmStashAllChangesOther')

    const items: IMenuItem[] = [
      {
        label: __DARWIN__
          ? t('changes.discardAllChangesDarwin')
          : t('changes.discardAllChangesOther'),
  ...
  private getOpenInExternalEditorMenuItem = (
    ...
    const { externalEditorLabel, t } = this.props

    const openInExternalEditor = externalEditorLabel
      ? t('changes.openInExternalEditor', { editor: externalEditorLabel })
      : getDefaultEditorLabel()
  ...
  private getDefaultContextMenu(
    ...
    if (paths.length === 1) {
      ...
      items.push({
        label: __DARWIN__
          ? t('changes.ignoreFileDarwin')
          : t('changes.ignoreFileOther'),
      ...
      if (pathComponents.length > 0) {
        ...
        items.push({
          label: __DARWIN__
            ? t('changes.ignoreFolderDarwin')
            : t('changes.ignoreFolderOther'),
          submenu,
          enabled,
        })
      }
      ...
      items.push({
        label: __DARWIN__
          ? t('changes.ignoreSelectedFilesDarwin', { count: paths.length })
          : t('changes.ignoreSelectedFilesOther', { count: paths.length }),
        action: () => {
    ...
    Array.from(extensions)
      ...
      .forEach(extension => {
        items.push({
          label: __DARWIN__
            ? t('changes.ignoreAllExtensionFilesDarwin', { extension })
            : t('changes.ignoreAllExtensionFilesOther', { extension }),
    ...
    if (paths.length > 1) {
      items.push(
        ...
        {
          label: __DARWIN__
            ? t('changes.includeSelectedFilesDarwin')
            : t('changes.includeSelectedFilesOther'),
          action: () => {
        ...
        {
          label: __DARWIN__
            ? t('changes.excludeSelectedFilesDarwin')
            : t('changes.excludeSelectedFilesOther'),
          action: () => {
  ...
  private getPlaceholderMessage(
    ...
    switch (firstFile.status.kind) {
      ...
      case AppFileStatusKind.Untracked:
        return t('changes.commitSummaryCreate', { fileName })
      case AppFileStatusKind.Deleted:
        return t('changes.commitSummaryDelete', { fileName })
      default:
        // TODO:
        ...
        return t('changes.commitSummaryUpdate', { fileName })
    }
  }
  ...
  private renderStashedChanges() {
    ...
    return (
      ...
      >
        ...
        <div className="text">{t('changes.stashedChanges')}</div>
  ...
  private renderCheckBoxRow = () => {
    const { workingDirectory, rebaseConflictState, isCommitting, t } =
      this.props
    ...
    const visibleCount = visibleFiles
    const checkAllLabel =
      visibleFiles !== files.length
        ? t('changes.visibleFilesOfCount', { visibleCount, count })
        : t('changes.changedFilesCount', { count })
  ...
  private getListAriaLabel = () => {
    ...
    const count = files.length
    return this.props.t('changes.changedFilesCount', { count })
  }

  public render() {
    const { workingDirectory, isCommitting, t } = this.props
  ...
  private renderHiddenChangesWarning = () => {
    ...
    return (
      <div className="hidden-changes-warning" id="hidden-changes-warning">
        ...
        <span className="sr-only">{t('common.warning')}</span>
        <span>{t('changes.hiddenChangesWillBeCommitted')}</span>
        <LinkButton onClick={this.showFilesToBeCommitted}>
          {t('changes.adjustFiltersToSeeAll', {
            count: filesSelected.length as number,
          })}
        </LinkButton>
      </div>
    )
  ...
  private renderNoChanges = () => {
    ...
    return (
      <div className="no-changes-filtered">
        ...
        <div className="title">
          {t('changes.noFilesMatchYourCurrentFilters')}
        </div>

        <div className="subtitle">
          {getNoResultsMessage(this.props.fileListFilter, t)}
        </div>
...
export const FilterChangesList = withTranslation()(FilterChangesListInternal)
