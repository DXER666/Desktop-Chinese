...
export class CommitMessageInternal extends React.Component<
  ICommitMessageProps & WithTranslation,
  ICommitMessageState
> {
  ...
  public async componentDidUpdate(
    ...
    if (
      ...
      this.setState({
        isCommittingStatusMessage: this.props.t(
          'commitMessage.committedJustNow',
          {
            summary: this.props.mostRecentLocalCommit.summary,
            shortSha: this.props.mostRecentLocalCommit.shortSha,
          }
        ),
      })
    }
  ...
  private get toggleCoAuthorsText(): string {
    ...
    return this.props.showCoAuthoredBy
      ? __DARWIN__
        ? t('commitMessage.removeCoAuthorsDarwin')
        : t('commitMessage.removeCoAuthorsOther')
      : __DARWIN__
      ? t('commitMessage.addCoAuthorsDarwin')
      : t('commitMessage.addCoAuthorsOther')
  }
  ...
  private getGenerateCommitMessageMenuItem(): IMenuItem | null {
    ...
    return {
      label: __DARWIN__
        ? t('commitMessage.generateCommitMessageWithCopilotDarwin')
        : t('commitMessage.generateCommitMessageWithCopilotOther'),
      action: () => {
  ...
  private getCommitSpellcheckEnabilityMenuItem(isEnabled: boolean): IMenuItem {
    ...
    const enableLabel = __DARWIN__
      ? t('commitMessage.enableCommitSpellcheckDarwin')
      : t('commitMessage.enableCommitSpellcheckOther')
    const disableLabel = __DARWIN__
      ? t('commitMessage.disableCommitSpellcheckDarwin')
      : t('commitMessage.disableCommitSpellcheckOther')
    return {
  ...
  private renderCopilotButton() {
    ...
    let ariaLabel = t('commitMessage.generateCommitMessage')
    ...
    if (!isGeneratingCommitMessage && noChangesAvailable) {
      ariaLabel += t('commitMessage.filesMustBeSelected')
    } else if (showCancelGenerateCommitMessage) {
      ariaLabel = t('commitMessage.cancelGenerating')
    } else if (isGeneratingCommitMessage) {
      ariaLabel = t('commitMessage.generatingCommitDetails')
    }
      ...
      <>
        ...
        >
          <AriaLiveContainer
            message={
              isGeneratingCommitMessage
                ? t('commitMessage.generatingCommitDetails')
                : ''
            }
          />
          <Octicon
  ...
  private renderCommitOptionsButton() {
    const { t } = this.props
    const ariaLabel = t('commitMessage.configureCommitOptions')
  ...
  private onCommitOptionsButtonClick = (
    ...
    if (enableHooksEnvironment()) {
      items.push({
        ...
        label: __DARWIN__
          ? t('commitMessage.bypassCommitHooksDarwin')
          : t('commitMessage.bypassCommitHooks'),
        action: () => {
    ...
    items.push({
      ...
      label: __DARWIN__
        ? t('commitMessage.addSignedOffByTrailerDarwin')
        : t('commitMessage.addSignedOffByTrailer'),
      action: () => {
        this.props.onUpdateCommitOptions(this.props.repository, {
    ...
    if (this.props.showAllowEmptyCommitOption) {
      items.push({
        ...
        label: __DARWIN__
          ? t('commitMessage.allowEmptyCommitDarwin')
          : t('commitMessage.allowEmptyCommit'),
        action: () => {
          this.props.onUpdateCommitOptions(this.props.repository, {
  ...
  private renderRuleFailurePopover() {
    ...
    const header = __DARWIN__
      ? t('commitMessage.commitMessageRuleFailuresDarwin')
      : t('commitMessage.commitMessageRuleFailuresOther')
    return (
      <Popover
      ...
      >
        ...
        <RepoRulesMetadataFailureList
          ...
          leadingText={t('commitMessage.thisCommitMessage')}
        />
      </Popover>
    )
  }
  ...
  private getButtonVerb() {
    const { isCommitting, commitToAmend, t } = this.props
  ...
  private getFilesToBeCommittedButtonText() {
    const { filesToBeCommittedCount, t } = this.props
    ...
    const pluralizedFile =
      filesToBeCommittedCount > 1
        ? t('commitMessage.files')
        : t('commitMessage.file')
  ...
  private getCommittingButtonTitle() {
    ...
    return `${verb} ${t('commitMessage.to')} ${branch}`
  }
  ...
  private getButtonTitle(): string {
    const { commitToAmend, commitButtonText, t } = this.props
    ...
    const isAmending = commitToAmend !== null
    return isAmending
      ? `${this.getButtonVerb()} ${t('commitMessage.lastCommit')}`
  ...
  private getButtonTooltip(buttonEnabled: boolean) {
    ...
    if (isSummaryBlank) {
      return t('commitMessage.commitSummaryRequired')
    } else if (
      !this.props.anyFilesSelected &&
      this.props.anyFilesAvailable &&
      !this.props.allowEmptyCommit
    ) {
      return t('commitMessage.selectFilesToCommit')
    } else if (this.props.isCommitting) {
      return t('commitMessage.committingChanges')
    }
  ...
  private renderSubmitButton() {
    const { isCommitting, isGeneratingCommitMessage, t } = this.props
    const isSummaryBlank = isEmptyOrWhitespace(this.summaryOrPlaceholder)
    const buttonEnabled =
      (this.canCommit() || this.canAmend()) &&
      !isCommitting &&
  ...
  private renderSummaryLengthHint(): JSX.Element | null {
    ...
    return (
      <ToggledtippedContent
        ...
        tooltip={
          <>
            <div className="title">{t('commitMessage.summaryLengthTitle')}</div>
            <div className="description">
              {t('commitMessage.summaryLengthDescription')}
            </div>
          </>
        }
        ariaLiveMessage={t('commitMessage.summaryLengthAria')}
        direction={TooltipDirection.NORTH}
        className="length-hint"
        tooltipClassName="length-hint-tooltip"
        ariaLabel={t('commitMessage.openSummaryLengthInfo')}
      >
  ...
  private renderRepoRuleCommitMessageFailureHint(): JSX.Element | null {
    ...
    let bypassMessage = ''
    if (canBypass) {
      ariaLabelPrefix = t('common.warning')
      bypassMessage = t('commitMessage.butYouCanBypass')
    } else {
      ariaLabelPrefix = t('common.error')
    }
    ...
    return (
      <button
        ...
        aria-label={`${ariaLabelPrefix}: ${t(
          'commitMessage.failsRepositoryRules',
          {
            bypassMessage,
          }
        )}`}
  ...
  private renderCommitProgress() {
    const { isCommitting, hookProgress, onShowCommitProgress, t } = this.props
    ...
    const text =
      hookName === 'pre-auto-gc' && status === 'finished'
        ? t('commitMessage.optimizingRepository')
        : status === 'started'
        ? t('commitMessage.hookRunning', { hookName })
        : status === 'finished'
        ? t('commitMessage.hookFinished', { hookName })
        : status === 'failed'
        ? t('commitMessage.hookFailed', { hookName })
    ...
    return (
      <div className={cn}>
        ...
        {onShowCommitProgress && (
          <Button
            tooltip={t('commitMessage.showCommitProgress')}
  ...
  public render() {
    ...
    return (
      <div
        role="group"
        aria-label={t('commitMessage.createCommit')}
        className={className}
      ...
      >
          ...
          <AutocompletingInput
            ...
            label={
              this.props.showInputLabels === true
                ? t('commitMessage.summary')
                : undefined
            }
            screenReaderLabel={t('commitMessage.commitSummary')}
        ...
        >
          <AutocompletingTextArea
            ...
            screenReaderLabel={
              this.props.showInputLabels !== true
                ? t('commitMessage.commitDescription')
                : undefined
            }
...
export const CommitMessage = withTranslation()(CommitMessageInternal)
