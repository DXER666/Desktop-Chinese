...
class CommitMessageAvatarInternal extends React.Component<
  ICommitMessageAvatarProps & WithTranslation,
...
> {
  ...
  public constructor(props: ICommitMessageAvatarProps & WithTranslation) {
    super(props)
  ...
  public render() {
    const { warningType, user, t } = this.props

    let ariaLabel = ''
    switch (warningType) {
      case 'none':
        ariaLabel = t(
          'commitAuthor.viewInformation',
      ...
      case 'misattribution':
        ariaLabel = t(
          'commitAuthor.misattributedWarning',
      ...
      case 'disallowedEmail':
        ariaLabel = t(
          'commitAuthor.disallowedEmailWarning',
  ...
  private renderGitConfigPopover() {
    ...
    const location = isGitConfigLocal
      ? t('commitAuthor.local', 'local')
      : t('commitAuthor.global', 'global')
    const locationDesc = isGitConfigLocal
      ? t('commitAuthor.forYourRepository', 'for your repository')
      : ''
    const settingsName = __DARWIN__
      ? t('common.settings', 'settings')
      : t('common.options', 'options')
    const repositorySettingsLabel = t(
      'commitAuthor.repositorySettings',
      'repository settings'
    )
    const settings = isGitConfigLocal
      ? repositorySettingsLabel
      : t('commitAuthor.gitSettingsName', `git {{settingsName}}`, {
          settingsName,
        })
    const buttonText = __DARWIN__
      ? t('commitAuthor.openGitSettingsDarwin', 'Open Git Settings')
      : t('commitAuthor.openGitSettingsOther', 'Open git settings')
    ...
    return (
      <>
        <p>
          {user &&
            user.name &&
            t('commitAuthor.emailLine', 'Email: {{email}}', {
              email: user.email,
            })}
        </p>
  ...
  private renderWarningPopover() {
    const { warningType, emailRuleFailures, t } = this.props

    const updateEmailTitle = __DARWIN__
      ? t('commitAuthor.updateEmailDarwin', 'Update Email')
      : t('commitAuthor.updateEmailOther', 'Update email')

    const sharedHeader = (
      <>
        {t(
          'commitAuthor.globalGitEmailPrefix',
          'The email in your global Git config ('
        )}
        <span className="git-email">{this.props.email}</span>
        {t('commitAuthor.globalGitEmailSuffix', ')')}
      </>
    )
    ...
    const sharedFooter = (
      <>
        {hasEmails && (
          <Row>
            <Select
              label={t('commitAuthor.yourAccountEmails', 'Your Account Emails')}
        ...
        <Row>
          <div className="secondary-text">
            {t(
              'commitAuthor.chooseLocalEmailInstruction',
              'You can{{also}} choose an email local to this repository from the ',
              {
                also: hasEmails ? t('commitAuthor.also', ' also') : '',
              }
            )}
            <LinkButton onClick={this.onRepositorySettingsClick}>
              {repositorySettingsLabel}
            </LinkButton>
            {t('commitAuthor.chooseLocalEmailInstructionSuffix', '.')}
          </div>
        </Row>
        <Row className="button-row">
          <Button onClick={this.onIgnoreClick} type="button">
            {t('commitAuthor.ignore', 'Ignore')}
          </Button>
          {hasEmails && (
    ...
    if (warningType === 'misattribution') {
      const accountTypeSuffix = this.props.isEnterpriseAccount
        ? ` ${t('commitAuthor.enterprise', 'Enterprise')}`
        : ''

      const userName =
        this.props.user && this.props.user.name
          ? t('commitAuthor.forUserName', ' for {{userName}}', {
      ...
      const learnMore = (
        <LinkButton
          ariaLabel={t(
            'commitAuthor.learnMoreAttributionAria',
        ...
        <>
          <Row>
            <div>
              {sharedHeader}{' '}
              {t(
                'commitAuthor.misattributionMessageBody',
                'does not match your GitHub{{accountTypeSuffix}} account{{userName}}.',
                { accountTypeSuffix, userName }
              )}{' '}
              {learnMore}
            </div>
          </Row>
  ...
  private getCommittingAsTitle(): string | JSX.Element | undefined {
    ...
    if (user === undefined) {
      return t('commitAuthor.unknownUser', 'Unknown user')
    }
    ...
    if (name) {
      return (
        <>
          {t('commitAuthor.committingAsNamePrefix', 'Committing as ')}
          <strong>{name}</strong>
        </>
      )
    }
    ...
    return (
      <>
        {t('commitAuthor.committingWithEmail', 'Committing with {{email}}', {
          email,
        })}
      </>
    )
 ...
 */
export const CommitMessageAvatar = withTranslation()(
  CommitMessageAvatarInternal
) as unknown as React.ComponentClass<ICommitMessageAvatarProps>
