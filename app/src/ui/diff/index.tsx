...
class DiffInternal extends React.Component<
  IDiffProps & WithTranslation,
  IDiffState
> {
  public constructor(props: IDiffProps & WithTranslation) {
    super(props)
    ...
    return (
      <div className="panel empty large-diff">
        ...
        <div className="description">
          <p>
            {t(
              'diff.tooLargeDefault',
              'The diff is too large to be displayed by default.'
            )}
          </p>
          <p>
            {t(
              'diff.tooLargePerformanceWarning',
        ...
        <Button onClick={this.showLargeDiff}>
          {__DARWIN__
            ? t('diff.showDiffDarwin', 'Show Diff')
            : t('diff.showDiffOther', 'Show diff')}
        </Button>
      </div>
    )
  }
...
/** A component which renders a diff for a file. */
export const Diff = withTranslation()(
  DiffInternal
) as unknown as React.ComponentClass<IDiffProps>
