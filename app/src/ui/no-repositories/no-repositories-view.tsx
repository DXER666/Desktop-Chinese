 ...
 */
class NoRepositoriesViewInternal extends React.Component<
  INoRepositoriesProps & WithTranslation,
...
> {
  ...
  public constructor(props: INoRepositoriesProps & WithTranslation) {
    super(props)
  ...
  public render() {
    ...
    return (
      <UiView id="no-repositories">
        <section aria-label={t('noRepositories.headingAriaLabel')}>
          <header>
            <h1>{t('noRepositories.title')}</h1>
            <p>{t('noRepositories.subtitle')}</p>
          </header>
  ...
  private renderTutorialRepositoryButton() {
    ...
    if (this.props.tutorialPaused) {
      return this.renderButtonGroupButton(
        ...
        __DARWIN__
          ? t('noRepositories.tutorialButtonResumeDarwin')
          : t('noRepositories.tutorialButtonResumeOther'),
      ...
      return this.renderButtonGroupButton(
        octicons.mortarBoard,
        __DARWIN__
          ? t('noRepositories.tutorialButtonCreateDarwin')
          : t('noRepositories.tutorialButtonCreateOther'),
  ...
  private renderCloneButton() {
    ...
    return this.renderButtonGroupButton(
      octicons.repoClone,
      __DARWIN__
        ? t('noRepositories.cloneButtonDarwin')
        : t('noRepositories.cloneButtonOther'),
      this.onShowClone,
      undefined,
      !this.isUserSignedIn()
    )
  ...
  private renderCreateRepositoryButton() {
    ...
    return this.renderButtonGroupButton(
      octicons.plus,
      __DARWIN__
        ? t('noRepositories.createButtonDarwin')
        : t('noRepositories.createButtonOther'),
      this.props.onCreate
    )
  }
  ...
  private renderAddExistingRepositoryButton() {
    ...
    return this.renderButtonGroupButton(
      ...
      __DARWIN__
        ? t('noRepositories.addButtonDarwin')
        : t('noRepositories.addButtonOther'),
      this.props.onAdd
    )
  }
    ...
    return (
      <div className="content-pane">
        ...
        <div className="drag-drop-info">
          ...
          <div>
            <strong>{t('noRepositories.proTipTitle')}</strong>{' '}
            {t('noRepositories.proTipContent')}
          </div>
        </div>
      </div>
    )
...
/**
 * 用 withTranslation 包装后导出的欢迎页组件。
 */
export const NoRepositoriesView = withTranslation()(
  NoRepositoriesViewInternal
) as React.ComponentClass<INoRepositoriesProps>
