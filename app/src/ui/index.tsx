...
// In Windows dev we prefer to use the system Git installation under
// C:\Program Files\Git over dugite's embedded (usually missing) package.
// Fallback order: explicit env var > bundled (__dirname/git) > system default.
;(function resolveAndSetGitEnv() {
  const defaultSystemDir = 'C:\\Program Files\\Git'
  const fallback = Path.resolve(__dirname, 'git')
  let dir: string | undefined =
    (typeof window !== 'undefined' && (window as any).__LOCAL_GIT_DIRECTORY) ||
  ...
  // 优先使用内置 git：它包含 git-credential-desktop.exe 等 GitHub Desktop
  // 专用工具，避免使用系统 git 时因缺少 credential-desktop helper 导致
  // 推送/拉取报 "git: 'credential-desktop' is not a git command"。
  // 系统 git 仅在内置 git 缺失时作为兜底。
  if (!dir) {
    try {
      ...
      /* no bundled git, fall back to system install below */
    }
  }
  if (!dir) {
    try {
      if (Fs.statSync(defaultSystemDir).isDirectory()) {
        dir = defaultSystemDir
      }
    } catch {
      /* no system install */
    }
  ...
  // Compute GIT_EXEC_PATH based on LOCAL_GIT_DIRECTORY.
  // Do NOT trust pre-existing GIT_EXEC_PATH as it may point to dugite's
  // non-existent embedded location. See https://git.io/JJ7KF
  const execPath = Path.join(dir, 'mingw64', 'libexec', 'git-core')
  try {
  ...
  // Keep LESSCHARSET=utf-8 for Chinese commit messages / paths.
