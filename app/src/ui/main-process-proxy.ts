...
/** 缓存最近一次发送的菜单标签。语言切换后若无新状态，可用来重发以刷新菜单翻译。 */
let lastMenuLabels: MenuLabelsEvent | null = null

/**
 * 计算当前要发送给主进程的菜单语言代码。
 * 优先用 i18next.resolvedLanguage（当前激活的），其次从存储的偏好解析。
 */
function getCurrentMenuUiLanguage(): string | undefined {
 ...
 *  额外注入当前界面语言代码，让主进程据此翻译菜单项为中文或英文。 */
export function updatePreferredAppMenuItemLabels(labels: MenuLabelsEvent) {
  const uiLanguage = getCurrentMenuUiLanguage()
  const enriched: MenuLabelsEvent = { ...labels, uiLanguage }
  lastMenuLabels = enriched
  ipcRenderer.send('update-preferred-app-menu-item-labels', enriched)
}

/**
 * 程序内语言切换完成后，用缓存的上一份 labels（注入新 uiLanguage）
 * 立刻重发一次，确保系统菜单即时切换语言。
 * 若从未发送过 labels，则返回 false，需要等 AppStore 初始化后的首次发送。
 */
export function refreshMenuLabelsWithCurrentLanguage(): boolean {
  if (lastMenuLabels == null) {
    return false
  }
  const uiLanguage = getCurrentMenuUiLanguage()
  const enriched: MenuLabelsEvent = { ...lastMenuLabels, uiLanguage }
  lastMenuLabels = enriched
  ipcRenderer.send('update-preferred-app-menu-item-labels', enriched)
  return true
}

// 订阅 i18n 语言变更：任何方式切换语言后都立刻刷新系统菜单
i18next.on('languageChanged', () => {
  refreshMenuLabelsWithCurrentLanguage()
})
