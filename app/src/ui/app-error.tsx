 ...
 */
class AppErrorImpl extends React.Component<
  IAppErrorProps & WithTranslation,
  IAppErrorState
> {
  private dialogContent: HTMLDivElement | null = null

  public constructor(props: IAppErrorProps & WithTranslation) {
    super(props)
    this.state = {
      error: props.error,
      disabled: false,
  ...
  public componentWillReceiveProps(nextProps: IAppErrorProps & WithTranslation) {
    ...
    if (
      ...
      return (
        <>
          ...
          {files.length > 0 && (
            <>
              <p>{t('appError.filesExceedingLimit')}</p>
              <ul>
                {files.map(file => (
                  <li key={file}>{file}</li>
                ))}
  ...
  private getTitle(error: Error) {
    ...
    switch (getDugiteError(error)) {
      case DugiteError.PushWithFileSizeExceedingLimit:
        return t('appError.titleFileSizeLimitExceeded')
    }

    switch (getRetryActionType(error)) {
      case RetryActionType.Clone:
        return t('appError.titleCloneFailed')
      case RetryActionType.Push:
        return t('appError.titlePushFailed')
    }
    ...
    if (isErrorWithMetaData(error)) {
      ...
      switch (gitContext?.kind) {
        case 'create-repository':
          return t('appError.titleCreateRepoFailed')
        case 'commit':
          return t('appError.titleCommitFailed')
      }
    }

    return t('appError.titleGeneric')
  }
  ...
  private renderContentAfterErrorMessage(error: Error) {
    ...
    if (retryAction && retryAction.type === RetryActionType.Clone) {
      ...
      return (
        <p>
          {t('appError.retryClonePrefix')}{' '}
          <Ref>{retryAction.name}</Ref>
          {t('appError.retryCloneSuffix')}
        </p>
      )
    }
  ...
  private renderOpenPreferencesFooter() {
    ...
    return (
      <DialogFooter>
        <OkCancelButtonGroup
          okButtonText={t('appError.close')}
          onOkButtonClick={this.onCloseButtonClick}
          cancelButtonText={
            __DARWIN__
              ? t('appError.openPreferences')
              : t('appError.openOptions')
          }
  ...
  private renderDefaultFooter() {
    return (
      <DefaultDialogFooter
        buttonText={this.props.t('appError.close')}
...
export const AppError = withTranslation()(AppErrorImpl) as any
