...
interface ICustomIntegrationFormProps {
  ...
  readonly labelArguments?: string
  readonly placeholderPath?: string
  readonly placeholderArguments?: string
  readonly chooseButtonText?: string
  readonly invalidPathErrorPrefix?: string
  readonly invalidPathErrorSuffix?: string
  readonly argsMissingTargetPathError?: string
  readonly invalidArgsError?: string
}
...
> {
  ...
  public render() {
    const {
      ...
      placeholderPath = 'Path to executable',
      placeholderArguments = 'Command-line arguments',
    ...
    return (
      <div className="custom-integration-form-container">
        <div className="custom-integration-form-path-container">
          ...
          />
          <Button onClick={this.onChoosePath}>{chooseButtonText}</Button>
        </div>
        ...
        <TextBox
          ...
          placeholder={placeholderArguments}
  ...
  private renderPathErrors() {
    ...
    const {
      invalidPathErrorPrefix = 'Unable to find an executable at ',
      invalidPathErrorSuffix = '. Please choose a valid path.',
    } = this.props

    const errorDescription =
      invalidPathErrorPrefix + this.state.path + invalidPathErrorSuffix
  ...
  private renderArgsErrors() {
    ...
    const {
      argsMissingTargetPathError = 'Arguments must include a target path ({argument})',
      invalidArgsError = 'These arguments are not valid.',
    } = this.props

    const errorDescription = this.state.showNonValidArgsError
      ? invalidArgsError
      : argsMissingTargetPathError.replace('{argument}', TargetPathArgument)
