...
export class CopilotUserSettingsInternal extends React.Component<
  ICopilotUserSettingsProps & WithTranslation
> {
  ...
  private renderModelPicker(): JSX.Element {
    const { t, copilotModels, byokProviders } = this.props

    if (copilotModels === null) {
      return <p>{t('preferences.copilot.loadingAvailableModels')}</p>
    }

    if (!hasCopilotModelPickerItems(copilotModels, byokProviders)) {
      return <p>{t('preferences.copilot.noModelsAvailable')}</p>
    }
    ...
    return (
      <>
        ...
        {this.renderFeatureModelPicker(
          ...
          'commit-message-generation',
          __DARWIN__
            ? t('preferences.copilot.commitMessageGenerationDarwin')
            : t('preferences.copilot.commitMessageGenerationOther'),
        ...
        <p className="settings-description">
          <LinkButton uri="https://docs.github.com/en/desktop/making-changes-in-a-branch/committing-and-reviewing-changes-to-your-project-in-github-desktop#write-a-commit-message-and-push-your-changes">
            {t('preferences.copilot.learnMoreAboutGeneratingCommitMessages')}
          </LinkButton>
        </p>
        {enableCopilotConflictResolution() && (
          <>
            {this.renderFeatureModelPicker(
              ...
              'conflict-resolution',
              __DARWIN__
                ? t('preferences.copilot.conflictResolutionDarwin')
                : t('preferences.copilot.conflictResolutionOther'),
              this.onConflictResolutionModelChanged,
              280
            )}
            <p className="settings-description">
              {t(
                'preferences.copilot.modelChangesApplyToFutureConflictResolutions'
              )}
            </p>
            <Checkbox
              label={
                __DARWIN__
                  ? t(
                      'preferences.copilot.alwaysUseCopilotWhenConflictsAreDetectedDarwin'
                    )
                  : t(
                      'preferences.copilot.alwaysUseCopilotWhenConflictsAreDetectedOther'
                    )
              }
              value={
...
export const CopilotUserSettings = withTranslation()(CopilotUserSettingsInternal)
