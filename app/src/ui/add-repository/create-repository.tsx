...
class CreateRepositoryInternal extends React.Component<
  ICreateRepositoryProps & WithTranslation,
...
> {
  ...
  public constructor(props: ICreateRepositoryProps & WithTranslation) {
    super(props)
  ...
  private updateDefaultDirectory = () => {
    ...
    if (!this.props.initialPath && this.state.path !== null) {
      setRepositoryDefaultPath(this.state.path)
    }
  }
    ...
    return (
      <Row>
        <Select
          label={
            __DARWIN__
              ? t('createRepository.gitIgnoreDarwin')
              : t('createRepository.gitIgnoreOther')
          }
  ...
  private renderLicenses() {
    ...
    return (
      <Row>
        <Select
          label={t('createRepository.licenseLabel')}
    ...
    return (
      <Row>
        <InputError
          ...
          ariaLiveMessage={`${t(
            'createRepository.existingRepoError1'
          )}${fullPath}${t('createRepository.existingRepoError2')}${t(
            'createRepository.existingRepoError3'
          )}${t('createRepository.existingRepoError4')}`}
        >
          {t('createRepository.existingRepoError1')}
          <Ref>{fullPath}</Ref>
          {t('createRepository.existingRepoError2')}
          <LinkButton onClick={this.onAddRepositoryClicked}>
            {t('createRepository.existingRepoError3')}
          </LinkButton>
          {t('createRepository.existingRepoError4')}
        </InputError>
      </Row>
    ...
    return (
      <Row>
        <InputWarning
          ...
          trackedUserInput={fullPath}
          ariaLiveMessage={`${t(
            'createRepository.subfolderWarning1'
          )}${fullPath}${t('createRepository.subfolderWarning2')}${t(
            'createRepository.subfolderWarning3'
          )}`}
        >
          {t('createRepository.subfolderWarning1')}
          <Ref>{fullPath}</Ref>
          {t('createRepository.subfolderWarning2')}
          <LinkButton uri={submoduleDocsUrl}>
            {t('createRepository.subfolderWarning3')}
          </LinkButton>
        </InputWarning>
      </Row>
    )
    ...
    return (
      <Row>
        <InputWarning
          ...
          ariaLiveMessage={`${t('createRepository.readmeOverwrite1')}${t(
            'createRepository.readmeOverwrite2'
          )}${t('createRepository.readmeOverwrite3')}`}
        >
          {t('createRepository.readmeOverwrite1')}
          <Ref>{t('createRepository.readmeOverwrite2')}</Ref>
          {t('createRepository.readmeOverwrite3')}
        </InputWarning>
      </Row>
    )
  ...
  private renderPathMessage = () => {
    ...
    return (
      <div id="create-repo-path-msg">
        {t('createRepository.createAtMsg1')}
        <Ref>{fullPath}</Ref>
        {t('createRepository.createAtMsg2')}
      </div>
    )
  }
  ...
  public render() {
    ...
    return (
      <Dialog
        ...
        title={
          __DARWIN__
            ? t('createRepository.titleDarwin')
            : t('createRepository.titleOther')
        }
      ...
      >
        ...
        <DialogContent>
          ...
          <Row>
            <TextBox
              ...
              label={t('createRepository.descriptionLabel')}
        ...
        <DialogFooter>
          ...
          <OkCancelButtonGroup
            okButtonText={
              __DARWIN__
                ? t('createRepository.okButtonDarwin')
                : t('createRepository.okButtonOther')
            }
...
/**
 * 用 withTranslation 包装后导出的创建仓库对话框组件。
 */
export const CreateRepository = withTranslation()(
  CreateRepositoryInternal
) as React.ComponentClass<ICreateRepositoryProps>
